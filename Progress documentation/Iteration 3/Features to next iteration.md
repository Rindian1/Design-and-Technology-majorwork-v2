Overall changes 
    ...
    3) Reorgnaise the heading bar, so that it goes 'Home' in the center, to the left of this it should be 'goals' and 'plugs', and to the right should be 'general insights and appliance specific' *quick fix*

Info popups
    1) info popups still dont work, when the user scrolls down and clicks a popup, they are not anchored properly, causing for the popup to be less readable. *quick fix*
    2) Feature popups, which describe each graph and stat. For example home page main graph should have a info, with a 'feature' tag, which says 'this shows your daily spending on energy', and the graph below should similarily have a feature tage, explainign waht it does. This should be for all features. *quick fix*
...
Goals page 
    1) Goals should be organised so that the goals selected by the user *quick fix*
    2) Add a notifications feature whenever a goal is finished, or failed. *HIGH IMPORTANCE*  
    3) Make the demo mode progress faster. *quick fix*\
...
11) Buy an AI api key for openai or similar, and reimplement a chatbot feature for the app. *HIGH IMPORTANCE* 

12) Make it so that goals that have been completed get replaced with an updated goal, of the same category but with slightly higer difficulty. For example if 'stay under budget for this week' is ompleted its replaced with 'stay under 2% of your budget for this week'. Should be very slight changes each time *quick fix* 